...
class Header(Flowable):
    ...
    def draw(self):
        ptext = "<font size=10><b>Statement Date: {}</b></font>".format("01/01/2017")
...
class EOB:
    ...
    def create_claims(self):
        ...
        member_rate = Paragraph(
            "<font size={}>Member<br/>rate</font>".format(fsize), self.styles["Normal"]
        )
        pending = Paragraph(
            "<font size={}>Pending or<br/>not payable<br/>(remarks)</font>".format(
                fsize
            ),
            self.styles["Normal"],
        )
